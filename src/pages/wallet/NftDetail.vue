...
const isNFTloading = computed<LoadingState>(
  () => ControllerModule.isNFTloading,
);
...
async function fetchPopularNfts() {
  if (nfts.value.length === 0 && exploreNFTS.value.length === 0) {
    try {
      ...
      const collections: NFTCollection[] = await get(
        `${config.api}/tokeninfo/popularNFTs`,
      );
...
<template>
  <div class="flex flex-col w-full py-2">
    ...
    <template v-else>
      <span class="text-app-text-500 nft-title"
        >You have {{ nfts.length }} NFTs</span
      >
      <div
        v-if="nfts.length === 0"
        class="w-full shadow dark:shadow-dark bg-white dark:bg-app-gray-700 rounded-md mt-10 p-12 pt-8"
      >
        <span
          class="text-app-text-500 dark:text-app-text-dark-400 text-center inline-block"
          >{{ getNftFetchMessage(exploreNFTSFetchState) }}</span
        >
        <RoundLoader
          v-if="exploreNFTSFetchState == 'loading'"
          class="w-10 h-10 mx-auto mb-4"
          color="border-white"
        />
        <div
          v-if="exploreNFTS.length"
          class="flex flex-wrap justify-center mt-12"
        >
          <div
            v-for="collection in exploreNFTS"
            :key="collection.image"
            class="flex flex-col items-center m-4 w-48 popular-nft"
          >
            <img
      ...
      <div
        v-else
        class="w-full mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 items-center justify-items-center"
      >
        <NftCard
          v-for="nft in nfts"
          :key="nft.mintAddress"
