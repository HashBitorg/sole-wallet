...
const routeName =
  router.currentRoute.value.name === "walletTopUp"
    ? TOPUP.MOONPAY
    : router.currentRoute.value.name;
const selectedProvider = ref<TopUpProvider>(
  topupPlugin[routeName?.toString() || TOPUP.MOONPAY],
);
        ...
        <div class="space-y-4">
          ...
          >
            ...
            >
                ...
                <div class="mr-3">
                  ...
                  >
                    <circle
                      ...
                      cy="12"
                      r="11"
                      :stroke="getBrandColor() || `currentColor`"
                    />
                    <circle
                      v-if="checked"
                      cx="12"
                      cy="12"
                      r="8"
                      :fill="getBrandColor() || `currentColor`"
                    />
                  </svg>
                </div>
                <img
                  :src="provider.logo(controllerModule.isDarkMode)"
                  :alt="provider.name"
                  class="w-24"
                />
              </div>
              <RadioGroupDescription
                as="div"
                class="col-span-1 whitespace-pre-wrap"
              >
                <div
                  class="text-right font-medium text-xs text-app-text-600 dark:text-app-text-dark-500"
                >
                ...
                <div
                  class="text-right ml-1 text-xs text-app-text-600 dark:text-app-text-dark-500 md:ml-0"
                >
                ...
                >
                  <span class="font-bold">{{
                    $t("walletTopUp.currencies")
                  }}</span
                  >:
                  {{
                    provider.validCryptocurrencies
                      .map((k) => k.value)
                      .join(", ")
                  }}
                </div>
